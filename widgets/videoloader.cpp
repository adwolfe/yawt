...
VideoLoader::VideoLoader(QWidget* parent)
    ...
    setMouseTracking(
        true);  // Enable for hover effects later if needed for tracks
    updateCursorShape();
}
...
// (Existing getters: isVideoLoaded, getTotalFrames, getFPS, etc. are assumed to
// be implemented)
...
// --- Control Slots ---
// (loadVideo, play, pause, seekToFrame, setZoomFactor*, clearRoi,
// setPlaybackSpeed, threshold setters as before)
// ... (Implementations for these are assumed to be present and correct from
// previous versions) ...
bool VideoLoader::loadVideo(const QString& filePath) {
    ...
    clearWormSelections();
    clearDisplayedTracks();  // Clear tracks on new video
    m_isPanning = false;
    m_isDefiningRoi = false;
    m_playbackSpeedMultiplier = 1.0;
    m_showThresholdMask = false;
    ...
    if (!openVideoFile(filePath)) { /* ... error handling ... */
        return false;
    }
    ...
    totalFramesCount =
        static_cast<int>(videoCapture.get(cv::CAP_PROP_FRAME_COUNT));
    originalFrameSize =
        QSize(static_cast<int>(videoCapture.get(cv::CAP_PROP_FRAME_WIDTH)),
              static_cast<int>(videoCapture.get(cv::CAP_PROP_FRAME_HEIGHT)));
    if (totalFramesCount <= 0 ||
        originalFrameSize.isEmpty()) { /* ... error handling ... */
        return false;
    }
    seekToFrame(0);
    emit videoLoaded(filePath, totalFramesCount, framesPerSecond,
                     originalFrameSize);
...
bool VideoLoader::openVideoFile(
    const QString& filePath) { /* ... as before ... */
    if (videoCapture.isOpened()) videoCapture.release();
    try {
        return videoCapture.open(filePath.toStdString());
    } catch (const cv::Exception& ex) {
        qWarning() << "OpenCV exception:" << ex.what();
        return false;
    }
}
...
void VideoLoader::seekToFrame(int frameNumber,
                              bool suppressEmit) { /* ... as before ... */
    if (!isVideoLoaded()) return;
    frameNumber =
        qBound(0, frameNumber, totalFramesCount > 0 ? totalFramesCount - 1 : 0);
    displayFrame(frameNumber, suppressEmit);
}
void VideoLoader::setZoomFactor(double factor) {
    setZoomFactorAtPoint(factor, rect().center());
}
void VideoLoader::setZoomFactorAtPoint(
    double factor, const QPointF& widgetPoint) { /* ... as before ... */
    if (!isVideoLoaded()) return;
    ...
    if (videoPointBeforeZoom.x() < 0 || videoPointBeforeZoom.y() < 0) {
        ...
        m_panOffset.setX((ws.width() - targetRectNow.width()) / 2.0 -
                         currentTopLeft.x());
        m_panOffset.setY((ws.height() - targetRectNow.height()) / 2.0 -
                         currentTopLeft.y());
    } else {
...
void VideoLoader::setThresholdAlgorithm(
    ThresholdAlgorithm algorithm) { /* ... as before ... */
...
void VideoLoader::setAssumeLightBackground(
    bool isLight) { /* ... as before ... */
...
void VideoLoader::setAdaptiveThresholdBlockSize(
    int blockSize) { /* ... as before ... */
...
void VideoLoader::setTracksToDisplay(const AllWormTracks& tracks) {
    ...
    qDebug() << "VideoLoader: Tracks set for display. Count:"
             << m_allTracksToDisplay.size();
}
...
void VideoLoader::setVisibleTrackIDs(const QSet<int>& visibleTrackIDs) {
    ...
    update();  // Trigger repaint
    qDebug() << "VideoLoader: Visible track IDs updated. Count:"
             << m_visibleTrackIDs.size();
}
...
// --- displayFrame, processNextFrame, convertCvMatToQImage (as before, ensure
// applyThresholding is always called in displayFrame) ---
void VideoLoader::displayFrame(int frameNumber, bool suppressEmit) {
    ...
    if (currentPos != frameNumber &&
        !(currentPos == frameNumber - 1 && m_isPlaying)) {
        if (!videoCapture.set(cv::CAP_PROP_POS_FRAMES,
...
void VideoLoader::convertCvMatToQImage(const cv::Mat& mat,
                                       QImage& qimg) { /* ... as before ... */
    if (mat.empty()) {
        qimg = QImage();
        return;
    }
...
void VideoLoader::paintEvent(QPaintEvent* event) {
    ...
    if (m_currentMode == InteractionMode::DrawROI && !m_activeRoiRect.isNull() &&
        m_activeRoiRect.isValid()) {
        QPointF roiTopLeftWidget = mapPointFromVideo(m_activeRoiRect.topLeft());
        QPointF roiBottomRightWidget =
            mapPointFromVideo(m_activeRoiRect.bottomRight());
        if (roiTopLeftWidget.x() >= 0 && roiBottomRightWidget.x() >= 0) {
            painter.setPen(QPen(Qt::red, 1, Qt::DashLine));
            painter.drawRect(
                QRectF(roiTopLeftWidget, roiBottomRightWidget).normalized());
        }
    }
    ...
    if ((m_currentMode == InteractionMode::DrawROI ||
        ...
        painter.drawRect(
            QRect(m_roiStartPointWidget, m_roiEndPointWidget).normalized());
    }

    // Draw selected worm centroids and bounding boxes for feedback (SelectWorms
    // mode)
    ...
    if (m_currentMode == InteractionMode::ViewEditTracks &&
        !m_allTracksToDisplay.empty()) {
        painter.setRenderHint(QPainter::Antialiasing,
                              true);  // Smoother lines for tracks

        for (int trackId : qAsConst(m_visibleTrackIDs)) {
            if (m_allTracksToDisplay.count(
                    trackId)) {  // Check if trackId exists in the map
                const std::vector<WormTrackPoint>& trackPoints =
                    m_allTracksToDisplay.at(trackId);
                ...
                for (size_t i = 0; i < trackPoints.size(); ++i) {
                    ...
                    // Only draw points that are relevant to the current view (e.g.,
                    // visible frames) For simplicity now, draw all points of visible
                    // tracks. Optimization: only draw segments if at least one end is on
                    // screen.

                    QPointF currentPointVideo(pt.position.x, pt.position.y);
                    QPointF currentPointWidget = mapPointFromVideo(currentPointVideo);

                    if (currentPointWidget.x() < 0)
                        continue;  // Skip if point is off-screen after mapping
    ...
    if (event->button() == Qt::LeftButton) {
        if (m_currentMode == InteractionMode::PanZoom) { /* ... as before ... */
            ...
            event->accept();
                   ...
                   m_currentMode == InteractionMode::Crop) { /* ... as before ... */
            ...
            if (videoCoords.x() >= 0) {
                ...
                m_isDefiningRoi = false;
                   ...
                   InteractionMode::SelectWorms) { /* ... as before ... */
            ...
            double minDistanceSq =
                TRACK_POINT_CLICK_TOLERANCE * TRACK_POINT_CLICK_TOLERANCE;
            ...
            if (bestTrackId != -1) {
                qDebug() << "Track point clicked: Worm ID" << bestTrackId << "Frame"
                         << bestFrameNum;
...
void VideoLoader::mouseReleaseEvent(
    QMouseEvent* event) { /* ... as before ... */
    if (!isVideoLoaded()) {
    ...
    if (event->button() == Qt::LeftButton) {
        if (m_isPanning) {
            ...
            updateCursorShape();
            event->accept();
        } else if (m_isDefiningRoi && (m_currentMode == InteractionMode::DrawROI ||
                                       m_currentMode == InteractionMode::Crop)) {
            m_roiEndPointWidget = event->pos();
            m_isDefiningRoi = false;
...
// updateCursorShape, clampPanOffset, handleRoiDefinedForCrop, performVideoCrop,
// applyThresholding, updateTimerInterval, emitThresholdParametersChanged,
// calculateTargetRect, mapPointToVideo, mapPointFromVideo as before
// ... (ensure these functions are present from previous updates) ...
void VideoLoader::updateCursorShape() {
    ...
    case InteractionMode::Crop:
        ...
        break;
    case InteractionMode::SelectWorms:
        setCursor(Qt::PointingHandCursor);
        break;
    case InteractionMode::ViewEditTracks:
        setCursor(Qt::ArrowCursor);
        break;  // Or PointingHand if tracks are "hot"
    default:
        setCursor(Qt::ArrowCursor);
        break;
    }
...
QPointF VideoLoader::mapPointToVideo(
    const QPointF& wp) const { /* ... as before ... */
...
QPointF VideoLoader::mapPointFromVideo(
    const QPointF& vp) const { /* ... as before ... */
    if (currentQImageFrame.isNull() || originalFrameSize.isEmpty() ||
...
void VideoLoader::clampPanOffset() { /* ... as before ... */
    ...
    QPointF centerOffset((widgetSz.width() - targetRect.width()) / 2.0,
                         (widgetSz.height() - targetRect.height()) / 2.0);
...
void VideoLoader::handleRoiDefinedForCrop(
    const QRectF& cropRoiVideoCoords) { /* ... as before ... */
...
bool VideoLoader::performVideoCrop(
    ...
    QString& outCroppedFilePath) { /* ... as before ... */
...
QColor VideoLoader::getTrackColor(int trackId) const {
    ...
    // For more distinct colors, you might use a predefined list or a more
    // sophisticated generator. QRandomGenerator is better than rand() for this.
    // Using a static generator to get different colors across calls if this
    // function is called multiple times for new IDs. However, for consistency
    // across runs for the same ID, caching is better. If m_trackColors is
    // mutable, we can add it here. For now, let's make it const and assume colors
    // are pre-assigned or this is just for demo. A better approach would be for
    // MainWindow to assign colors and pass them with the tracks. For a quick
    // solution:
    ...
    QColor color = QColor::fromHsv(hue, qBound(0, saturation, 255),
                                   qBound(0, lightness, 255));
    // To cache (requires m_trackColors to be non-const or use a const_cast if
    // really needed, not ideal):
