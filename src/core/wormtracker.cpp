                                            ...
                                            QRectF& currentRoi)
{
    ...
    if (isSplit) {
        ...
        TRACKING_DEBUG().noquote() << getDebugLabel("handleBoundaryTouchingBlob") << "handleBoundaryTouchingBlob - SPLIT detected with " << splitCandidates.size() << " candidates";
                                         ...
                                         QRectF& currentRoi)
{
    ...
    if (asMerged) {
        ...
        if (!touchesBoundary) {
            ...
            bool confirmedMerge = detectMergeByAreaIncrease(searchCandidate);
            if (confirmedMerge) {
                TRACKING_DEBUG().noquote() << getDebugLabel("handleMultipleBlobsCase")
...
{
    TRACKING_DEBUG().noquote() << getDebugLabel("detectSplitByAreaReduction") << "detectSplitByAreaReduction - CALLED with currentArea:" << currentBlob.area;

    if (!m_lastFullBlob.isValid) {
        TRACKING_DEBUG().noquote() << getDebugLabel("detectSplitByAreaReduction") << "detectSplitByAreaReduction - No lastFullBlob, returning false";
        return false;
    }
    ...
    TRACKING_DEBUG().noquote() << getDebugLabel("detectSplitByAreaReduction")
...
{
    TRACKING_DEBUG().noquote() << getDebugLabel("detectMergeByAreaIncrease") << "detectMergeByAreaIncrease - CALLED with currentArea:" << currentBlob.area;

    if (m_skipMergeDetectionNextFrame) {
        TRACKING_DEBUG().noquote() << getDebugLabel("detectMergeByAreaIncrease") << "detectMergeByAreaIncrease - Skipping merge detection this frame";
        return false;
    }
    ...
    if (m_lastPrimaryBlob.isValid &&
        ...
        isMerge = true;
        TRACKING_DEBUG().noquote() << getDebugLabel("detectMergeByAreaIncrease")
    ...
    TRACKING_DEBUG().noquote() << getDebugLabel("detectMergeByAreaIncrease") << "detectMergeByAreaIncrease - Result isMerge:" << isMerge;
