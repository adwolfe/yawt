...
bool AppController::getPreferredWormPosition(int wormId, int frame, QPointF& outPos, QRectF& outRoi) const
{
    if (!m_storage) return false;
    // Prefer exact-frame tracked position
    if (m_storage->getWormDataForFrame(wormId, frame, outPos, outRoi)) {
        return true;
    }
    // Fallback to last known position before the requested frame
    if (m_storage->getLastKnownPositionBefore(wormId, frame, outPos, outRoi)) {
        return true;
    }
    return false;
}

QList<AppController::MergeSplitEvent> AppController::getMergeSplitEventsForWorm(int wormId, int maxFrame) const
{
    QList<MergeSplitEvent> events;
    if (!m_storage) return events;
    if (maxFrame <= 0) return events;

    QList<int> prevGroup;
    for (int f = 0; f < maxFrame; ++f) {
        QList<QList<int>> groups = m_storage->getMergeGroupsForFrame(f);
        QList<int> currGroup;
        for (const QList<int>& g : groups) {
            if (g.contains(wormId)) { currGroup = g; break; }
        }

        QSet<int> prevSet; for (int id : prevGroup) prevSet.insert(id);
        QSet<int> currSet; for (int id : currGroup) currSet.insert(id);
        if (prevSet == currSet) { prevGroup = currGroup; continue; }
        ...
        if (prevEmpty && !currEmpty) {
            eventType = "Merge";
            QSet<int> others = currSet; others.remove(wormId);
            if (others.isEmpty()) details = "Merged (no other ids)";
            else {
                QStringList ids; for (int id : others) ids << QString::number(id);
                details = QString("Merged with worm(s) %1").arg(ids.join(", "));
            }
        } else if (!prevEmpty && currEmpty) {
            eventType = "Split";
            QSet<int> others = prevSet; others.remove(wormId);
            ...
            if (!added.isEmpty() && removed.isEmpty()) {
                eventType = "Merge";
                QSet<int> others = added; others.remove(wormId);
                QStringList ids; for (int id : others) ids << QString::number(id);
                details = QString("Merged with worm(s) %1").arg(ids.join(", "));
            } else if (added.isEmpty() && !removed.isEmpty()) {
                eventType = "Split";
                QSet<int> others = removed; others.remove(wormId);
                QStringList ids; for (int id : others) ids << QString::number(id);
                details = QString("Split from worm(s) %1").arg(ids.join(", "));
            } else {
                eventType = "Merge/Split";
                QStringList parts;
                if (!added.isEmpty()) { QStringList ids; for (int id : added) ids << QString::number(id); parts << QString("Added: %1").arg(ids.join(", ")); }
                if (!removed.isEmpty()) { QStringList ids; for (int id : removed) ids << QString::number(id); parts << QString("Removed: %1").arg(ids.join(", ")); }
                details = parts.join("; ");
            }
        }
